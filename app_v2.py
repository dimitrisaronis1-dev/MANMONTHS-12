...
class Model:
    ...
    def __init__(self):
        ...
        # request granularity: integer requests must allocate/deallocate in full months (10 steps)
        self.row_integer_request: Dict[int, bool] = {}
        self.row_granularity_steps: Dict[int, int] = {}
    ...
    def donor_can_deallocate(self, r: int) -> bool:
        # Rule 3: keep >= one granularity unit if possible
        g = self.row_granularity_steps.get(r, 1)
        return self.row_steps_count.get(r, 0) > g
...
def _steps_owned_in_month(model: Model, r: int, ym: Tuple[int, int]) -> int:
    """How many 0.1-step slots row r owns in month ym."""
    return model.month_slots.get(ym, []).count(r)


def move_block(model: Model, ws, r: int, from_ym: Tuple[int, int], to_ym: Tuple[int, int], steps: int) -> bool:
    """
    Move `steps` step-slots for row r from from_ym to to_ym.
    Used for integer-granularity rows (typically 10 steps = 1.0 AM).
    Destination must have enough free capacity. For full-month moves (10), we require destination to be empty.
    """
    if steps <= 0:
        return False
    if from_ym not in model.ym_to_col or to_ym not in model.ym_to_col:
    ...
    if to_ym not in model.row_months.get(r, set()):
        return False
    if _steps_owned_in_month(model, r, from_ym) < steps:
        return False
    # enforce full-month integrity for 10-step moves
    if steps >= Model.UNITS_PER_AM and model.month_total_steps(to_ym) != 0:
        return False
    if model.month_total_steps(to_ym) + steps > Model.MAX_MONTHLY_CAPACITY_STEPS:
        return False
    if to_ym[0] != from_ym[0] and model.year_total_steps(to_ym[0]) + steps > Model.MAX_YEARLY_CAPACITY_STEPS:
        return False

    for _ in range(steps):
        if not move_x(model, ws, r, from_ym, to_ym):
            return False
    return True


def make_month_empty(model: Model, ws, ym: Tuple[int, int], depth: int = 0, max_depth: int = 10, visited=None) -> bool:
    """
    Ensure month `ym` becomes completely empty (0/10 steps).
    This is required to place a full-month (1.0 AM) allocation.

    Strategy:
    - For decimal rows: move steps out one-by-one (can chain).
    - For integer-granularity rows (10-step blocks): move the whole block to another empty month within its allowed period.
    """
    if model.month_total_steps(ym) == 0:
        return True
    if visited is None:
        visited = set()
    key = (ym, depth, 'empty')
    ...
    # Work on unique occupants, prefer whites first then yellows
    occupants_unique = list(dict.fromkeys(model.month_slots.get(ym, [])))
    occupants_unique.sort(key=lambda r: (model.is_yellow_row(r), -model.row_steps_count.get(r, 0)))

    for occ_r in occupants_unique:
        if model.month_total_steps(ym) == 0:
            return True
        g = model.row_granularity_steps.get(occ_r, 1)
        owned = _steps_owned_in_month(model, occ_r, ym)

        # if row has only one eligible month, we cannot move it
        if len(model.row_months_list.get(occ_r, [])) <= 1:
            continue

        if g >= Model.UNITS_PER_AM and owned >= Model.UNITS_PER_AM:
            # move full-month block to another EMPTY month in its allowed set
            dests = [d for d in model.row_months_list.get(occ_r, []) if d != ym]
            # prefer same year months first
            dests.sort(key=lambda d: (d[0] != ym[0], d[1]))
            moved = False
            for to_ym in dests:
                # destination must be empty; if not, try to empty it (chain)
                if model.month_total_steps(to_ym) != 0:
                    if not make_month_empty(model, ws, to_ym, depth + 1, max_depth, visited):
                        continue
                if move_block(model, ws, occ_r, ym, to_ym, Model.UNITS_PER_AM):
                    moved = True
                    break
            if moved:
                # restart scan since month_slots changed
                return make_month_empty(model, ws, ym, depth, max_depth, visited)
        else:
            # decimal: use existing step-level freeing (one step at a time)
            if model.month_total_steps(ym) > 0:
                if make_month_free(model, ws, ym, depth=depth, max_depth=max_depth, visited=visited):
                    return make_month_empty(model, ws, ym, depth, max_depth, visited)

    return model.month_total_steps(ym) == 0

def try_steal_clear_month(model: Model, ws, ym: Tuple[int, int], allow_yellow: bool) -> bool:
    """Clear ALL allocations from month ym by stealing them (no relocation).
    Preferably used only for Rule 3 (avoid 0 allocations). If allow_yellow is False,
    only steal from white rows.
    """
    if model.month_total_steps(ym) == 0:
        return True

    # Determine donors and how many steps each owns in this month
    donors = list(dict.fromkeys(model.month_slots.get(ym, [])))
    donor_steps = {d: _steps_owned_in_month(model, d, ym) for d in donors}

    if not allow_yellow:
        if any(model.is_yellow_row(d) for d in donors):
            return False

    # Check each donor can give away its steps in this month without violating Rule 3 for the donor
    for d in donors:
        steps_here = donor_steps.get(d, 0)
        if steps_here <= 0:
            continue
        # donor minimum depends on its own granularity (integer-request rows must keep >=10 if possible, decimals >=1)
        d_min = model.row_granularity_steps.get(d, 1)
        if model.row_steps_count.get(d, 0) - steps_here < d_min:
            return False

    # Perform the steals (remove steps from each donor)
    for d in donors:
        steps_here = donor_steps.get(d, 0)
        if steps_here <= 0:
            continue
        if not remove_steps(model, ws, d, ym, steps_here):
            return False

    return model.month_total_steps(ym) == 0


def try_steal_clear_month_rule3(model: Model, ws, ym: Tuple[int, int], allow_yellow: bool) -> bool:
    """Clear ALL allocations from month ym by stealing them, even if a donor becomes 0.
    This is only for Rule 3 (avoid zero-allocation rows). If allow_yellow is False, only steal from white rows.
    Donors that are reduced to 0 will be handled in subsequent Rule 3 passes if possible.
    """
    if model.month_total_steps(ym) == 0:
        return True

    donors = list(dict.fromkeys(model.month_slots.get(ym, [])))
    if not allow_yellow and any(model.is_yellow_row(d) for d in donors):
        return False
...
def add_x(model: Model, ws, r: int, ym: Tuple[int, int], steps: int = 1) -> bool:
    """
    v2: add steps (1 step = 0.1 AM) to row r at month ym.
    For integer-request rows, callers must pass steps=10 and the month must be empty.
    """
    if steps <= 0:
        return False
    if ym not in model.row_months.get(r, set()):
        return False
    # enforce granularity rule
    g = model.row_granularity_steps.get(r, 1)
    if steps % g != 0:
        return False
    if model.month_total_steps(ym) + steps > Model.MAX_MONTHLY_CAPACITY_STEPS:
        return False
    if model.year_total_steps(ym[0]) + steps > Model.MAX_YEARLY_CAPACITY_STEPS:
        return False
    # integer rows can only use empty months
    if g > 1 and model.month_total_steps(ym) != 0:
        return False

    model.month_slots.setdefault(ym, []).extend([r] * steps)
    model.yearly_totals_steps[ym[0]] = model.year_total_steps(ym[0]) + steps
    model.row_steps_count[r] = model.row_steps_count.get(r, 0) + steps
    _cell_add_step(ws, r, model.ym_to_col[ym], +steps)
    return True


def remove_steps(model: Model, ws, donor_row: int, ym: Tuple[int, int], steps: int) -> bool:
    """Remove EXACTLY steps from donor_row in ym (if present)."""
    if steps <= 0:
        return False
    slots = model.month_slots.get(ym, [])
    # donor must have at least steps occurrences in this month
    if slots.count(donor_row) < steps:
        return False
    # enforce donor granularity
    g = model.row_granularity_steps.get(donor_row, 1)
    if steps % g != 0:
        return False
    # perform removals
    removed = 0
    i = 0
    while i < len(slots) and removed < steps:
        if slots[i] == donor_row:
            slots.pop(i)
            removed += 1
            continue
        i += 1
    if removed != steps:
        return False
    model.yearly_totals_steps[ym[0]] = model.year_total_steps(ym[0]) - steps
    model.row_steps_count[donor_row] = model.row_steps_count.get(donor_row, 0) - steps
    _cell_add_step(ws, donor_row, model.ym_to_col[ym], -steps)
    return True


def remove_one_step(model: Model, ws, donor_row: int, ym: Tuple[int, int]) -> bool:
    """Backwards-compat: remove 1 step only for decimal rows."""
    if model.row_granularity_steps.get(donor_row, 1) != 1:
        return False
    return remove_steps(model, ws, donor_row, ym, 1)


def free_capacity_in_year(model: Model, ws, year: int) -> bool:
    """
    Try to free yearly capacity for `year`.

    IMPORTANT (v2+integer rule):
    - Decimal rows can move/deallocate 1 step (0.1 AM) at a time.
    - Integer-request rows MUST move/deallocate in FULL months (10 steps) to remain integer-safe.

    Strategy (prefer white, then yellow only as last resort):
    1) Move allocation OUT of `year` to another allowed month (possibly in another year).
       - Decimal rows: move 1 step.
       - Integer rows: move 10 steps to a completely empty destination month (month_total==0).
    2) If moving is impossible, deallocate from a donor in `year` while preserving minimum granularity.
       - Decimal rows: remove 1 step only if donor keeps >=1 step.
       - Integer rows: remove 10 steps only if donor keeps >=10 steps.
    """

    def steps_owned_in_month(r: int, ym) -> int:
        return sum(1 for owner in model.month_slots.get(ym, []) if owner == r)

    def move_steps(r: int, ym_from, ym_to, steps: int) -> bool:
        for _ in range(steps):
            if not move_x(model, ws, r, ym_from, ym_to):
                return False
        return True

    def remove_steps(r: int, ym_from, steps: int) -> bool:
        for _ in range(steps):
            if not remove_one_step(model, ws, r, ym_from):
                return False
        return True

    # Collect unique (ym,row) that have allocation in this year, with owned step counts
    pairs = []
    for ym, owners in model.month_slots.items():
        if ym[0] != year:
            continue
        # unique rows for this month
        for r in set(owners):
            cnt = steps_owned_in_month(r, ym)
            if cnt > 0:
                pairs.append((ym, r, cnt))

    # prefer stealing/moving from white first, then yellow
    pairs.sort(key=lambda t: (model.is_yellow_row(t[1]), model.row_steps_count.get(t[1], 0), -t[2]))

    # 1) Move OUT of this year
    for allow_yellow in (False, True):
        for ym_from, r, cnt in pairs:
            ...
            # must own enough steps in this month to move a full granularity chunk
            if cnt < steps_to_move:
                continue
            ...
            for ym_to in model.row_months_list.get(r, []):
                ...
                if model.year_total_steps(ym_to[0]) + steps_to_move > Model.MAX_YEARLY_CAPACITY_STEPS:
                    continue

                if steps_to_move > 1:
                    # integer move requires completely empty destination month
                    if model.month_total_steps(ym_to) != 0:
                        continue
                else:
                    if model.month_total_steps(ym_to) >= Model.MAX_MONTHLY_CAPACITY_STEPS:
                        continue

                if move_steps(r, ym_from, ym_to, steps_to_move):
                    return True

    # 2) Deallocate from donors in this year (white first; yellow only if no white option)
    for allow_yellow in (False, True):
        for ym_from, r, cnt in pairs:
            if (not allow_yellow) and model.is_yellow_row(r):
                continue
            ...
            # preserve minimum granularity (Rule 3)
            if model.row_steps_count.get(r, 0) <= steps_to_remove:
                continue

            # do not touch yellow unless explicitly in the yellow pass
            if model.is_yellow_row(r) and not allow_yellow:
                continue

            if cnt < steps_to_remove:
                continue

            if remove_steps(r, ym_from, steps_to_remove):
                return True

    return False
def make_month_free(model: Model, ws, ym: Tuple[int, int], depth: int = 0, max_depth: int = 6, visited=None) -> bool:
    """
    Ensure month `ym` has at least 1 free step slot (total < 10).

    Enhancement:
    - If month is full and contains an INTEGER-only owner (granularity 10),
      try to MOVE that full month (10 steps) to another EMPTY month within that row's allowed range.
      This preserves the rule: integer-request rows never end up with a decimal (<1) month.
    - Otherwise, fallback to relocating ONE decimal step (0.1) from an occupant to another allowed month (chain-capable).
    """
    ...
    # Helper: steps owned by row r in month ym
    def owned_steps(r: int, ym_) -> int:
        return sum(1 for o in model.month_slots.get(ym_, []) if o == r)

    unique_rows = list(dict.fromkeys(occupants))

    # 1) Try moving INTEGER owners out in full-month blocks
    # Prefer white first, then yellow (moving yellow is allowed; stealing yellow is last resort elsewhere)
    unique_rows.sort(key=lambda r: (model.is_yellow_row(r), -model.row_steps_count.get(r, 0)))
    for r in unique_rows:
        g = model.row_granularity_steps.get(r, 1)
        if g <= 1:
            continue
        if owned_steps(r, ym) < g:
            continue

        # destination order: same-year first then other years
        dests_same = [d for d in model.row_months_list.get(r, []) if d != ym and d[0] == ym[0]]
        dests_other = [d for d in model.row_months_list.get(r, []) if d != ym and d[0] != ym[0]]

        for to_ym in dests_same + dests_other:
            # must be completely empty to keep integer block intact
            if model.month_total_steps(to_ym) != 0:
                continue

            # ensure destination year capacity
            if model.year_total_steps(to_ym[0]) + g > Model.MAX_YEARLY_CAPACITY_STEPS:
                # try freeing capacity in that year
            ...
            ok = True
            for _ in range(g):
                if not move_x(model, ws, r, ym, to_ym):
                    ok = False
                    break
            if ok:
                return model.month_total_steps(ym) < Model.MAX_MONTHLY_CAPACITY_STEPS

    # 2) Fallback: relocate ONE decimal step (chain-capable)
    # Prefer white donors with more total steps, then yellow
    occupants.sort(key=lambda r: (model.is_yellow_row(r), -model.row_steps_count.get(r, 0)))
    for occ_r in occupants:
        g = model.row_granularity_steps.get(occ_r, 1)
        # Allow moving 0.1 even from rows that started as integer-request,
        # as long as they keep at least 1.0 AM total after the move.
        if model.row_steps_count.get(occ_r, 0) <= Model.UNITS_PER_AM:
            continue
...
def process_excel(input_bytes: bytes, input_filename: str = "") -> Tuple[bytes, str]:
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        req_steps = model.row_requested_steps[r]
        model.row_integer_request[r] = (req_steps > 0 and req_steps % Model.UNITS_PER_AM == 0)
        model.row_granularity_steps[r] = Model.UNITS_PER_AM if model.row_integer_request[r] else 1
        model.row_is_yellow[r] = bool(row["is_yellow"])
        model.row_project_index[r] = idx
        model.row_steps_count[r] = 0

        ws.cell(r, 1).value = f"=MATCH(C{r},CV!$C$2:$C${last_row_b},0)"
        ws.cell(r, 1).border = thin_border

        ws.cell(r, 2).value = f"=VLOOKUP(A{r},CV!$A$2:$B$1899,2,FALSE)"
        ws.cell(r, 2).border = thin_border

        ws.cell(r, 3).value = model.row_period[r]
        ...
        for ym in model.row_months_list[r]:
            ...
            if g > 1:
                # integer request: allocate only full months (10 steps) in empty months
                if got + g <= need and model.month_total_steps(ym) == 0 and model.year_total_steps(ym[0]) + g <= Model.MAX_YEARLY_CAPACITY_STEPS:
                    if add_x(model, ws, r, ym, steps=g):
                        got += g
                continue

            # decimal request: fill month with 0.1 steps
            while got < need and model.month_total_steps(ym) < Model.MAX_MONTHLY_CAPACITY_STEPS and model.year_total_steps(ym[0]) < Model.MAX_YEARLY_CAPACITY_STEPS:
                if not add_x(model, ws, r, ym, steps=1):
                    break
                got += 1
    ...
    # Rule 3: ensure >= one granularity unit if possible
    for _pass in range(6):
        progress = False
        for r in list(model.row_period.keys()):
            req = model.row_requested_steps.get(r, 0)
            if req <= 0:
                continue
            if model.row_steps_count.get(r, 0) != 0:
                continue
            ...
            for ym in model.row_months_list.get(r, []):
                if g > 1:
                    # Integer-request rows can only be allocated in a completely empty month (0/10).
                    if model.month_total_steps(ym) != 0:
                        # First try to empty the month via chain moves (preferred).
                        if not make_month_empty(model, ws, ym, depth=0, max_depth=10, visited=set()):
                            # If still not possible, apply Rule 3 stealing:
                            # 1) steal only from white rows
                            if not try_steal_clear_month_rule3(model, ws, ym, allow_yellow=False):
                                # 2) last resort: steal from yellows too
                                if not try_steal_clear_month_rule3(model, ws, ym, allow_yellow=True):
                                    continue
                
                    while model.year_total_steps(ym[0]) + g > Model.MAX_YEARLY_CAPACITY_STEPS:
                        if not free_capacity_in_year(model, ws, ym[0]):
                            break
                    if model.year_total_steps(ym[0]) + g > Model.MAX_YEARLY_CAPACITY_STEPS:
                        continue
                    if add_x(model, ws, r, ym, steps=g):
                        satisfied = True
                        progress = True
                    ...
                    # Decimal rows: try to make room and allocate 0.1
                    if model.month_total_steps(ym) >= Model.MAX_MONTHLY_CAPACITY_STEPS:
                    ...
                    if model.year_total_steps(ym[0]) >= Model.MAX_YEARLY_CAPACITY_STEPS:
                        if not free_capacity_in_year(model, ws, ym[0]):
                            continue
                    if add_x(model, ws, r, ym, steps=1):
                        satisfied = True
                        progress = True
                        break
...
# Improve white deficits by controlled stealing from WHITE donors (>1 step)
    def improve_white_deficits_by_steal(max_iters: int = 8000) -> None:
        ...
        while it < max_iters:
            it += 1
            deficits = [rr for rr in white_rows if model.row_granularity_steps.get(rr, 1) == 1 and model.row_requested_steps.get(rr, 0) > model.row_steps_count.get(rr, 0)]
            if not deficits:
                break
                    ...
                    if model.month_total_steps(ym) >= Model.MAX_MONTHLY_CAPACITY_STEPS:
                        ...
                        for donor in owners:
                            ...
                            if model.is_yellow_row(donor):
                                continue
                            if model.row_granularity_steps.get(donor, 1) != 1:
                                continue
                            if not model.donor_can_deallocate(donor):
                                continue
                            # steal: remove from donor, add to rr (month total stays 10)
                            if remove_one_step(model, ws, donor, ym) and add_x(model, ws, rr, ym, steps=1):
                                stolen = True
                                progress = True
                                break
                        if stolen:
