...
def make_month_free(model: Model, ws, ym: Tuple[int, int], depth: int = 0, max_depth: int = 6, visited=None) -> bool:
    ...
    Strategy:
    1) Try moving a full 1.0 month (10 steps) out if occupant row is integer-granularity AND there is a fully empty destination month.
    2) Try relocating ONE step (0.1) from an occupant to another allowed month (chain-capable).
    3) If relocation is impossible, STEAL (remove) ONE step from an occupant in this month, preserving minimum row totals:
       - Prefer WHITE donors first, then YELLOW as last resort.
       - Never reduce a row's total below 1.0 AM if it has request >= 1 (Rule 3 expectation).
    """
    ...
    # 1) Move integer full-month blocks out to an empty month (preserves nice integer months when possible)
    ...
    # 3) Last resort: steal one step from an occupant in this month
    for allow_yellow in (False, True):
        for donor in unique_rows:
            if (not allow_yellow) and model.is_yellow_row(donor):
                continue

            # Preserve minimum of 1.0 total
            if model.row_steps_count.get(donor, 0) <= Model.UNITS_PER_AM:
                continue

            if remove_one_step(model, ws, donor, ym):
                return True
