...
def move_x(model: Model, ws, r: int, from_ym: Tuple[int, int], to_ym: Tuple[int, int]) -> bool:
    ...
    # Guard: do not split an integer (full) month (1.0 AM) into decimals (must remain 1.0)
    from_col = model.ym_to_col.get(from_ym)
    if from_col is not None:
        v_from = ws.cell(r, from_col).value
        try:
            if v_from is not None and float(v_from) == 1.0:
                return False
        except Exception:
            pass
...
def remove_one_step(model: Model, ws, donor_row: int, ym: Tuple[int, int]) -> bool:
    ...
    # Do NOT fractionalize full months of a project (keep integer 1.0 AM)
    if slots.count(donor_row) >= Model.MAX_MONTHLY_CAPACITY_STEPS:
        return False
    if donor_row not in slots:
        return False
    # Guard: do not take a step out of an integer (full) month (1.0 AM) -> must remain 1.0
    col = model.ym_to_col.get(ym)
    if col is not None:
        v = ws.cell(donor_row, col).value
        try:
...
def process_excel(input_bytes: bytes, input_filename: str = "") -> Tuple[bytes, str]:
    ...
    ws.row_dimensions[2].height = 30  # double height
    for rr in range(2, 5):
        for cc in range(1, 3):
