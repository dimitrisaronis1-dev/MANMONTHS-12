...
class Model:
    ...
    def donor_can_deallocate(self, r: int, ym: Tuple[int, int]) -> bool:
        """Whether we are allowed to remove ONE step (0.1 AM) from row r in month ym.

        Locked rule:
        - Rule 3: keep >= 1 step if possible (so donor must have > 1 total steps)
        v2 refinement:
        - If a project has ONLY ONE eligible month (its whole period maps to one month),
          then a FULL month allocation (10 steps) in that month must NOT be reduced to a decimal.
        """
        # Rule 3: keep >= 1 step if possible
        if self.row_steps_count.get(r, 0) <= 1:
            return False

        # Integer-request projects must never end up with fractional (<1) allocations.
        # Therefore, we never allow deallocating a single step from them.
        req_steps = self.row_requested_steps.get(r, 0)
        if req_steps > 0 and req_steps % self.UNITS_PER_AM == 0:
            return False

        # Prevent breaking a full month for single-month projects
        months_list = self.row_months_list.get(r, [])
        if len(months_list) == 1 and months_list[0] == ym:
            # count how many steps this row has in that specific month
            steps_in_month = self.month_slots.get(ym, []).count(r)
            if steps_in_month >= self.UNITS_PER_AM:  # 10 steps => 1.0 AM shown as "X"
                return False
...
def move_x(model: Model, ws, r: int, from_ym: Tuple[int, int], to_ym: Tuple[int, int]) -> bool:
    """
    v2: move steps from from_ym -> to_ym for the same row.

    Rule:
    - If requested AM is integer (requested_steps multiple of 10), we move 10 steps at a time (full month blocks).
    - Otherwise we move 1 step (0.1 AM) at a time.
    """
    ...
    block = Model.UNITS_PER_AM if (req_steps > 0 and req_steps % Model.UNITS_PER_AM == 0) else 1

    # must own enough steps in from_ym
    if model.month_slots.get(from_ym, []).count(r) < block:
        return False

    # destination must have capacity for the whole block
    if model.month_total_steps(to_ym) + block > Model.MAX_MONTHLY_CAPACITY_STEPS:
        return False

    # yearly capacity if changing year
    if to_ym[0] != from_ym[0] and model.year_total_steps(to_ym[0]) + block > Model.MAX_YEARLY_CAPACITY_STEPS:
        return False

    if not remove_one_step(model, ws, r, from_ym, steps=block):
        return False
    if not add_x(model, ws, r, to_ym, steps=block):
        # rollback
        add_x(model, ws, r, from_ym, steps=block)
        return False
    return True

def add_x(model: Model, ws, r: int, ym: Tuple[int, int], steps: int = 1) -> bool:
    """
    v2: add `steps` steps (each step = 0.1 AM) to row r at month ym.

    Important rule (v2):
    - If a project requested an integer number of AM (requested_steps is multiple of 10),
      we will call this with steps=10 only (full month blocks), never with steps=1.
    """
    if steps <= 0:
        return False
...
def remove_one_step(model: Model, ws, donor_row: int, ym: Tuple[int, int], steps: int = 1) -> bool:
    """Remove `steps` steps from donor_row in ym (if present)."""
    if steps <= 0:
        return False
    slots = model.month_slots.get(ym, [])
    if slots.count(donor_row) < steps:
        return False
    # remove `steps` occurrences
    removed = 0
    new_slots = []
    for rr in slots:
        if rr == donor_row and removed < steps:
            removed += 1
        else:
            new_slots.append(rr)
    model.month_slots[ym] = new_slots
    model.yearly_totals_steps[ym[0]] = model.year_total_steps(ym[0]) - steps
...
def process_excel(input_bytes: bytes, input_filename: str = "") -> Tuple[bytes, str]:
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        got = 0
        integer_req = (need > 0 and need % Model.UNITS_PER_AM == 0)
        ...
        for ym in model.row_months_list[r]:
            ...
            if integer_req:
                # allocate only FULL months (10 steps). This is allowed ONLY if the month can be fully reserved.
                if got + Model.UNITS_PER_AM > need:
                    continue
                # empty the month (no overlaps allowed for full month)
                while model.month_total_steps(ym) > 0:
                    if not make_month_free(model, ws, ym, depth=0, max_depth=6, visited=set()):
                        break
                # ensure enough yearly capacity (10 steps)
                while model.year_total_steps(ym[0]) > Model.MAX_YEARLY_CAPACITY_STEPS - Model.UNITS_PER_AM:
                    if not free_capacity_in_year(model, ws, ym[0]):
                        break
                if add_x(model, ws, r, ym, steps=Model.UNITS_PER_AM):
                    got += Model.UNITS_PER_AM
            else:
...
for c in range(START_COL, end_col):
    ...
    # Rule 3: ensure minimum allocation if possible
    for r in list(model.row_period.keys()):
        ...
        integer_req = (req % Model.UNITS_PER_AM == 0)
        min_steps = Model.UNITS_PER_AM if integer_req else 1

        satisfied = False
        for ym in model.row_months_list.get(r, []):
            # for integer requests we need the month fully reservable (no overlaps)
            if integer_req:
                while model.month_total_steps(ym) > 0:
                    if not make_month_free(model, ws, ym, depth=0, max_depth=6, visited=set()):
                        break
            ...
            # ensure enough yearly capacity
            while model.year_total_steps(ym[0]) > Model.MAX_YEARLY_CAPACITY_STEPS - min_steps:
                if not free_capacity_in_year(model, ws, ym[0]):
                    break

            if add_x(model, ws, r, ym, steps=min_steps):
                satisfied = True
                break
            ...
            for rr in deficits:
                for ym in model.row_months_list.get(rr, []):
                    ...
                    if model.year_total_steps(ym[0]) >= Model.MAX_YEARLY_CAPACITY_STEPS:
                        if not free_capacity_in_year(model, ws, ym[0]):
                            continue
                    req = model.row_requested_steps.get(rr, 0)
                    integer_req = (req > 0 and req % Model.UNITS_PER_AM == 0)
                    block = Model.UNITS_PER_AM if integer_req else 1
                    ...
                    # ensure enough yearly capacity
                    while model.year_total_steps(ym[0]) > Model.MAX_YEARLY_CAPACITY_STEPS - block:
                        if not free_capacity_in_year(model, ws, ym[0]):
                            break

                    if add_x(model, ws, rr, ym, steps=block):
                        changed = True
                        break
                if changed:
                    break
                    ...
                    if model.month_total_steps(ym) >= Model.MAX_MONTHLY_CAPACITY_STEPS:
                        ...
                        for donor in owners:
                            ...
                            if model.is_yellow_row(donor):
                                continue
                            if not model.donor_can_deallocate(donor, ym):
                                continue
